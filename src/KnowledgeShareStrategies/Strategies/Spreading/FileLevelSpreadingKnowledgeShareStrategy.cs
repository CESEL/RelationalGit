...
namespace RelationalGit
{
    ...
    public class FileLevelSpreadingKnowledgePickTopKShareStrategy : SpreadingKnowledgeShareStrategyBase
    {
        public FileLevelSpreadingKnowledgePickTopKShareStrategy(string knowledgeSaveReviewerReplacementType) : base(knowledgeSaveReviewerReplacementType)
        { }
        ...
        internal override IEnumerable<(string[] Reviewers, DeveloperKnowledge SelectedCandidateKnowledge)> GetPossibleCandidateSets(PullRequestContext pullRequestContext, DeveloperKnowledge[] availableDevs, PullRequestKnowledgeDistribution prereviewKnowledgeDistribution)
        {
            var takeCount = availableDevs.Length >= pullRequestContext.ActualReviewers.Length
                ? pullRequestContext.ActualReviewers.Length
                : availableDevs.Length;

            var topLeastExpertise = availableDevs.OrderBy(q=>q.NumberOfTouchedFiles).Take(takeCount).Select(q=>q.DeveloperName).ToArray();

            yield return (topLeastExpertise, null);
